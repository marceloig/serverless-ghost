...
```bash
serverless-ghost$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
serverless-ghost$ sam local start-api
serverless-ghost$ curl http://localhost:3000/
```
...
```bash
serverless-ghost$ sam logs -n HelloWorldFunction --stack-name serverless-ghost --tail
```
...
```bash
serverless-ghost$ cd hello-world
...
```bash
sam delete --stack-name serverless-ghost
```
